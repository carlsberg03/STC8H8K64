...
#include "stdio.h"
void oled_p6x8num(uint8 x,uint8 y,uint16 num)
{
  uint8 tmp[6],i;
  tmp[5]=0;
  tmp[4]=(uint8)(num%10+0x30);
  tmp[3]=(uint8)(num/10%10+0x30);
  tmp[2]=(uint8)(num/100%10+0x30);
  tmp[1]=(uint8)(num/1000%10+0x30);
  tmp[0]=(uint8)(num/10000%10+0x30);
...
//   char mytxt[50];
//   sprintf(mytxt,"%5d",num);
//   oled_p6x8str(x,y,(unsigned char *)mytxt);
}


void oled_p6x8float(uint8 x,uint8 y,float num)
{
   int num2,num3;
   char mytxt[50];
   uint8 sign=1;
   ...
   if(sign==-1)
   {
      sprintf(mytxt,"-%2d.%d",num3,num2);
   }
   else if(sign==1)
   {
      sprintf(mytxt,"%3d.%d",num3,num2);
   }
   oled_p6x8str(x,y,(unsigned char *)mytxt);
}

void oled_p6x8str_inv(uint8 x,uint8 y,uint8 ch[])
{
  uint8 c=0,i=0,j=0;      
  while (ch[j]!='\0')
  {
